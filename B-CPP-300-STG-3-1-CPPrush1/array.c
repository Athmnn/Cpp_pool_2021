...
static void ArrayIterator_setval(ArrayIteratorClass *this, ...)
{
    ...
    if (this->_idx >= this->_array->_size)
        raise("Out of range");
    va_start(ls, this);
    this->_array->_tab[this->_idx] = va_arg(ls, Class *);
    va_end(ls);
}
...
const Class    *ArrayIterator = (const Class *)&ArrayIteratorDescr;
...
static void Array_ctor(ArrayClass *this, va_list *args)
{
    ...
    this->_tab = malloc(this->_type->__size__ * this->_size + 1);
    if (!this->_tab)
        raise("Memory allocation failed.");
    for (; i < this->_size; ++i) {
        this->_tab[i] = va_new(this->_type, args);
        if (!this->_tab[i])
            raise("Memory allocation falied");
    }
    this->_tab[i] = NULL;
}
...
Object* Array_getitem(ArrayClass* this, ...)
{
    size_t idx;
    va_list ls;
...
void Array_setitem(ArrayClass* this, ...)
{
    size_t idx;
    va_list ls;
    ...
    if (idx > this->_size)
        raise("Idx is out of bounds");
    if (((Class *)this->_tab[idx])->__dtor__)
        ((Class *)this->_tab[idx])->__dtor__(this->_tab[idx]);
    if (((Class *)this->_tab[idx])->__ctor__)
        ((Class *)this->_tab[idx])->__ctor__(this->_tab[idx], &ls);
    va_end(ls);
}
